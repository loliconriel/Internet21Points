    ...
    while True:
        ...
        for client in rooms[room_choice]:
            if client == client_socket:
                while True:
                    client_socket.sendall("是否加牌？(y/n): ".encode())
                    choice = client_socket.recv(1024).decode().strip()
                    if choice.lower() == 'y':
                        card = deck.pop()
                        hands[client_socket].append(card)
                        client_socket.sendall(f"你抽到的牌是 {card}\n".encode())
                        client_socket.sendall(f"你的牌：{hands[client_socket]}\n".encode())
                        if calculate_score(hands[client_socket]) > 21:
                            client_socket.sendall("你爆牌了！\n".encode())
                            break
                    elif choice.lower() == 'n':
                        break
        ...
        # 是否繼續遊戲
        client_socket.sendall("是否重新開始？(y/n): ".encode())
        choice = client_socket.recv(1024).decode().strip()
        if choice.lower() != 'y':
